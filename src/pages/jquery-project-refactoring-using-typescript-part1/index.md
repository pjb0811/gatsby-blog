...
개발 환경에서 사용 중인 설정파일의 경우 업그레이드 된 버전에 맞게 `mode` 타입을 추가해주었고(운영 환경에 배포하는 작업은 아니기 때문에 운영 환경과 관련되 `mode`는 수정하지 않았다), 최신 버전의 가이드에 맞게 소스맵 설정을 수정했다. 그 외 업그레이드에 대한 기존 설정 파일에 대한 수정은 필요하지 않았다.
